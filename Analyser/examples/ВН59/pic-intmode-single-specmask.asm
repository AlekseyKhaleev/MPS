...
; Без каскадирования, режим прерываний
; Обработчики прерываний просто выдают свой номер на индикатор
; Обработчик IRQ0 выдает FFh
; Обработчик IRQ7 выполняется довольно долго,
; он показывает числа от 0 до 20 на большом индикаторе
; Используется автоцикл при завершении прерывания
; IRQ7 работает в режиме специальной маски
...
mvi a, 01101000b    ; установка специальной маски
sta pic_address0
...
mvi a, 01000000b  ; сброс специальной маски
sta pic_address0
